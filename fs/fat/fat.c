...
static int write_pbr(struct blk_desc *dev_desc,struct disk_partition *info)
{
	...
	img = malloc_cache_aligned(sizeof(__u8)*512);
	if(img == NULL) {
	...
	mk2(bsbpb->res, 32); /* Reserved Sector Count */
	...
	printf("Fat size : 0x%x\n", (unsigned int)(info->size / (spc * 128)));
	...
	/* Set Magic Number */
	mk2(img + 512 - 2, 0xaa55); /* Signature */
...
static int write_reserved(struct blk_desc *dev_desc,struct disk_partition *info)
{
	...
	mk4(img + 512 - 28, 0x61417272); /* Struct Signature */
	mk4(img + 512 - 24, 0xffffffff); /* Free Cluster Count */
	mk4(img + 512 - 20, 0x3); /* Next Free Cluster */
	mk2(img + 512 - 2, 0xaa55); /* Trail Signature */
...
fat_size)
{
	...
	dummy = malloc_cache_aligned(sizeof(__u8) * 8192);
	if(dummy == NULL) {
	...
	/* Write FAT Region */
	if (blk_dwrite(dev_desc, info->start + 32, 1, (ulong *)img) != 1) {
 ...
 */
int
fat_format_device(struct blk_desc *dev_desc, int part_no,struct disk_partition info)
{
