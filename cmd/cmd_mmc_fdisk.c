...
#define		DISK_START			(16 * 1024 * 1024)//mj (16*1024*1024) //same as raw area size
...
typedef struct cmd_tbl_t cmd_tbl_t;
...
int get_mmc_block_count(char *device_name)
{
	...
	blk_dev=blk_get_devnum_by_type(IF_TYPE_MMC, dev_num);
	block_count = (int)(mmc->capacity / blk_dev->blksz);
...
int get_mmc_mbr(char *device_name, unsigned char *mbr)
{
	...
	dev_num =(int)simple_strtoul(device_name, NULL, 0);
	mmc = find_mmc_device(dev_num);
	if (!mmc)
	{
		printf("mmc/sd device is NOT founded.\n");
	...
	blk_dev=blk_get_devnum_by_type(IF_TYPE_MMC, dev_num);
	rv = blk_dread(blk_dev,0,1,(ulong *)mbr);
	if(rv == 1)
		return 0;
	else
		return -1; 
...
int put_mmc_mbr(unsigned char *mbr, char *device_name)
{
	...
	dev_num = simple_strtoul(device_name, NULL, 0);
	mmc = find_mmc_device(dev_num);
	sddbg("find the dev-num %d,%s",dev_num,mmc->cfg->name);
	if (!mmc)
	{
		printf("mmc/sd device is NOT founded.\n");
		return -1;
	...
	blk_dev=blk_get_devnum_by_type(IF_TYPE_MMC, dev_num);
	rv =blk_dwrite(blk_dev, 0, 1,(ulong *)mbr);
	if(rv == 1)
		return 0;
	else
		return -1; 
...
int print_mmc_part_info(int argc, char *const argv[])
{
	...
	rv[0]=get_mmc_part_info(argv[2], 1, &(partInfo[0].block_start), &(partInfo[0].block_count),
			&(partInfo[0].partitionId) );
	
	rv[1]=get_mmc_part_info(argv[2], 2, &(partInfo[1].block_start), &(partInfo[1].block_count),
			&(partInfo[1].partitionId) );

	rv[2]=get_mmc_part_info(argv[2], 3, &(partInfo[2].block_start), &(partInfo[2].block_count),
			&(partInfo[2].partitionId) );

	rv[3]=get_mmc_part_info(argv[2], 4, &(partInfo[3].block_start), &(partInfo[3].block_count),
			&(partInfo[3].partitionId) );

	printf("\n");	
	if(!(rv[0]==-1 && rv[1]==-1 && rv[2]==-1 && rv[3]==-1))
	{
